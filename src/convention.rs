...
impl VersionValidator {
    ...
    pub fn validate(&self, version: &str) -> Result<()> {
        match self {
            VersionValidator::Semver => semver::Version::parse(version)
                .map(|_| Ok(()))
                .context(format!("Invalid semver version string: {version}"))?,
            VersionValidator::LenientSemver => lenient_semver::parse(version)
                .map(|_| Ok(()))
                .map_err(|e| e.owned())
                .context(format!("Invalid lenient semver version string: {version}"))?,
            VersionValidator::Pep440 => pep440::Version::is_canonical(version)
                .then(|| Ok(()))
                .context(format!("Invalid pep440 version string: {version}"))?,
            VersionValidator::AcceptAll => Ok(()),
        }
    }
}
